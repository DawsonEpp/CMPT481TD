...
func _check_turrets():
	...
	hovered_turret = null
	#print(get_tree().get_nodes_in_group("turrets"))

	# Get all turrets in the scene — adjust group name to match yours
	for turret in get_tree().get_nodes_in_group("turrets"):
		...
		if dist <= radius:
			...
			break  # pick closest if you want; for now first match wins
